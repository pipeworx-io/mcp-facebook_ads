...
This pack runs against a connected facebook_ads account, so it needs a Pipeworx key: sign in at https://pipeworx.io/account, connect facebook_ads, then call `POST https://gateway.pipeworx.io/v1/tools/fb_list_ad_accounts` with `Authorization: Bearer <your Pipeworx key>`. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/fb_list_ad_accounts`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "facebook_ads": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-facebook_ads"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-facebook_ads
```
